CREATE PROCEDURE [dbo].[uspPerfiles_Seleccion]
(
	--@OpcionSeleccion int, 
	@IdPerfil int, 
	@Activo int 
...
BEGIN
	...
	SELECT tPf.idperfil, 
		...
		tPf.fechacreacion, 
		tPf.idusuariocreo, 
		tUsC.claveusr as UsuarioCreo,
		tPf.fechaultmodif, 
		isnull(tPf.idusuarioultmodif, 0) as idusuarioultmodif, 
		isnull(tUsM.claveusr, '') as UsuarioModif,
        CASE tPf.activo WHEN 1 THEN 'Activo' WHEN 2 THEN 'No Activo' END as Estatus
	FROM tbperfiles as tPf
		INNER JOIN tbusuarios tUsC ON tPf.idusuariocreo = tUsC.idusuario
		LEFT JOIN tbusuarios tUsM ON tPf.idusuarioultmodif = tUsM.idusuario
	where tPf.idperfil = ISNULL(@IdPerfil,tPf.idperfil)
		and tPf.activo = ISNULL(@Activo, tPf.activo)
